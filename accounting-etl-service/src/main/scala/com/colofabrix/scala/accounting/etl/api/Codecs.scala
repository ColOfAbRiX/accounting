...
object CirceCodecs {
  ...
  private def newtEnc[NT, W](implicit gen: GenNewt[NT, W], wEnc: Enc[W]): Enc[NT] = new Enc[NT] {
    def apply(n: NT): Json = wEnc.apply(gen.to(n).head)
  }

  /** Circe decoder for newtype case classes (case classes with only one member) */
  private def newtDec[NT, W](implicit gen: GenNewt[NT, W], wDec: Dec[W]): Dec[NT] = new Dec[NT] {
